...
//brand renklerini vermek için brand isimlerinin yanına obje şeklinde random renkler ekleniyor

var filteredBrand = [];
var newArrOProducts = [];

oProducts.forEach((element) => {
  newArrOProducts.push(element.brand.toLowerCase());
});
newArrOProducts.forEach((element) => {
  let allKeys = filteredBrand.flatMap(Object.keys);
  if (allKeys.length) {
    var oExistedIndex = allKeys
      .map(function (oItem, oIndex) {
        return oItem;
      })
      .indexOf(element);
    ...
    if (oExistedIndex == -1) {
      ...
      if (filteredBrand[oExistedIndex][element].length < 1) {
        filteredBrand[oExistedIndex][element].push(
...
//sıralamaya göre dizi değiştiriliyor ve nesneler ona göre üretiliyor 
...
nesneOlustur(oProducts)
function varsayilan() {
  for (let i = 0; i < oProducts.length; i++) {
    let min = i;
    let j;
    for (j = i + 1; j < oProducts.length ; j++) {
      if (oProducts[j].id < oProducts[min].id) {
        min = j;
      }
    }
    let temp = oProducts[i];
    oProducts[i] = oProducts[min];
    oProducts[min] = temp;
    }
...
function artan() {
  ...
  let j;
  for (j = i + 1; j < oProducts.length ; j++) {
    if (oProducts[j].price < oProducts[min].price) {
      min = j;
    }
  }
  let temp = oProducts[i];
...
function azalan() {
  ...
  for (let i = 0; i < oProducts.length/2; i++) {
      let temp = oProducts[oProducts.length-1-i];
      oProducts[oProducts.length-1-i] = oProducts[i];
      oProducts[i] = temp;
  }
  for (let i = 0; i < oProducts.length; i++) {
    document.getElementsByClassName(`card`)[0].remove();
